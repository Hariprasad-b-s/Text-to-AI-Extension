...
(() => {
    ...
    function createButtons() {
        ...
        // Calculate position (bottom of selection, horizontally centered relative to selection)
        let x = rect.left + window.scrollX + (rect.width / 2) - 60; // Approximate center
        let y = rect.bottom + window.scrollY + 10;
        
        // Ensure it doesn't go off-screen
        x = Math.max(10, Math.min(x, document.body.scrollWidth - 150));
        ...
        btnClaude.title = 'Send to Claude';
        btnClaude.innerHTML = `
        ...
        btnChatGPT.title = 'Send to ChatGPT';
        ...
        btnGemini.title = 'Send to Gemini';
        btnGemini.innerHTML = `
          <span class="claude-jd-ext-icon">
    ...
    function setButtonProcessing(btn, text) {
        if (btn) {
            ...
            // Remove the text assignment since we are just using icons
            // const svg = btn.querySelector('svg').outerHTML;
            // btn.innerHTML = `<span class="claude-jd-ext-icon">${svg}</span>`;
        }
    }
    ...
    function handleSendToClaude(e) {
        ...
        if (!isValidSelection(selectedText)) {
            showToast('Selection is too short', true);
            return;
        }
        ...
        try {
            ...
            console.error(error);
            showToast('⚠️ Extension updated! Please refresh this page.', true);
        }
    }
